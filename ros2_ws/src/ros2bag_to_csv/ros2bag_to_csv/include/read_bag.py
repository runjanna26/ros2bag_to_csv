...
def read_write_from_all_topics(bag_file, print_out=False):
    ...
    for i in range(len(topic_names)):
        ...
        save_csv_file([timestamps, messages],path + "/" + bag_file + '-' + topic_name+".csv")

    # Close connection to the database
    close(connection)

def find_db3_file():
    directory_path = "."  # Change this to the directory where your .db3 file is located

    for filename in os.listdir(directory_path):
        if filename.endswith(".db3"):
            return filename

    return None  # If no .db3 file is found in the directory